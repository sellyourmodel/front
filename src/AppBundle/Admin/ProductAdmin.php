...
class ProductAdmin extends Admin
{
    ...
    public function createQuery($context = 'list')
    {
        ...
        if($request->isXmlHttpRequest() OR is_array($request->get('filter'))){
            ...
            if($categoryId){
                $category = $this->getConfigurationPool()->getContainer()->get('doctrine')->getRepository("AppBundle:Category")->find($categoryId);
    ...
    public function configureFormFields(FormMapper $formMapper)
    {
        ...
        $categoriesChoices = [];
        $categories = $this->getConfigurationPool()->getContainer()->get('doctrine')->getRepository("AppBundle:Category")->findBy(["parent"=>NULL],["pos"=>"ASC"]);
